...
- **Incident production (2026-07-29, post-déploiement) :** la migration `20260729190000` appliquée en prod a causé une récursion RLS infinie entre `grilles` et `parties` (ma nouvelle policy `grilles` interrogeait `parties` en clair ; "Créateur lit ses parties", Story 2.1, interroge déjà `grilles` en retour) -- panne totale (500 sur toute lecture de `grilles`, y compris pour le créateur). Non détecté par la revue (aucun accès à une base Supabase réelle pour l'exercer, limite déjà documentée). Corrigé en production via `drop policy` immédiat puis recréation encapsulée dans une fonction `SECURITY DEFINER` (`joueur_lit_cette_grille`, même pattern que `est_dans_la_partie`) -- reproduit dans le dépôt par une nouvelle migration `20260729213000_corriger_recursion_rls_grilles.sql` (jamais de réécriture de la migration déjà appliquée). **Leçon retenue : toute policy RLS touchant plusieurs tables doit être vérifiée pour un cycle de référence mutuel AVANT d'être appliquée en prod, pas seulement relue -- l'absence d'accès à une base réelle pendant le dev est un risque réel, pas juste une limite cosmétique, pour ce type de changement précis.**
- **Bug rapporté par l'utilisateur après redéploiement (2026-07-29) :** le bouton "Clôturer la Partie" restait visible pour un joueur non-créateur dans `GrilleEnDirecteScreen.tsx`. Cause : `estCreateur` se déduisait de la simple lisibilité de `grilles` ("si je peux lire cette ligne, je suis créateur") -- une hypothèse vraie avant CAP-6, invalidée par la nouvelle policy select qui autorise désormais N'IMPORTE QUEL joueur à lire sa grille. **Pas une faille exploitable** : la policy `update` réelle sur `parties` ("Créateur clôture sa partie", Story 2.1) compare `grilles.compte_id = auth.uid()` et n'a jamais été touchée -- un clic par un non-créateur échouait déjà silencieusement côté serveur, seul l'affichage du bouton était incorrect. Corrigé en comparant explicitement `grilles.compte_id` à `compte_id` de MA PROPRE ligne `joueurs` (jamais `auth.uid()` en direct, pour qu'un invité anonyme sans `compte_id` échoue aussi la comparaison) -- aucune nouvelle policy nécessaire, RLS existante suffit. Fix frontend uniquement, aucune migration additionnelle.
