...
export function CreationGrilleScreen({
  ...
  return grille ? (
    <ComposerPhrases
      grille={grille}
      onRetourBibliotheque={onRetourBibliotheque}
      onPartieLancee={onPartieLancee}
    />
  ) : (
    <NouvelleGrilleForm
      onCreated={setGrille}
...
function NouvelleGrilleForm({
  onCreated,
  onRetourBibliotheque,
}: NouvelleGrilleFormProps) {
  ...
  async function handleCreer() {
    ...
    try {
      const { data: nouvelleGrille, error: grilleError } =
        await grillesService.creerGrille(nom.trim(), taille);
      ...
      onCreated({
        id: nouvelleGrille.id,
        nom: nouvelleGrille.nom,
        taille: nouvelleGrille.taille,
      });
    } catch {
      if (nouvelleGrilleId) {
        await grillesService.annulerCreationGrille(nouvelleGrilleId);
    ...
    <main className="page creation-grille-screen">
      ...
      {taille !== null && (
        <div className="creation-grille-screen__phrase-container">
              ...
              <li key={index} className="phrase-list__item--input">
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setPhrases((current) =>
                      current.map((p, i) => (i === index ? valeur : p)),
                    );
                  }}
                />
              </li>
          ...
          {phrases.length < total && (
            <form
              className="creation-grille-screen__form"
              onSubmit={handleAjouterPhrase}
            >
              <input
                className="creation-grille-screen__input"
                type="text"
      ...
      <div className="page-action-bar">
        <Button
          type="button"
          variant="primary"
          disabled={pending || !nom.trim() || !complete}
          onClick={handleCreer}
        >
          Créer la grille
        </Button>
...
function ComposerPhrases({
  grille,
  onRetourBibliotheque,
  onPartieLancee,
}: ComposerPhrasesProps) {
  ...
  async function handleAjouter(event: FormEvent) {
    ...
    try {
      const { data, error } = await phrasesService.creerPhrase(
        grille.id,
        texte,
      );
  ...
  async function saveEdit(id: string) {
    ...
    try {
      ...
      setPhrases((current) =>
        current.map((p) => (p.id === id ? { ...p, texte } : p)),
      );
      setEditingId((current) => (current === id ? null : current));
    } catch {
      setMessage(friendlyErrorMessage());
  ...
  async function handleDupliquer() {
    ...
    try {
      const { data: nouvelleGrille, error: grilleError } =
        await grillesService.creerGrille(nomDeLaCopie(nom), taille);
  ...
  async function handleChangerTaille(nouvelleTaille: number) {
    ...
    try {
      const { error } = await grillesService.changerTailleGrille(
        grille.id,
        nouvelleTaille,
      );
      if (error) {
  ...
  if (chargementEchoue) {
    return (
      <main className="page">
        <p className="creation-grille-screen__message">
          {friendlyErrorMessage()}
        </p>
        <Button
          type="button"
  ...
  return (
    <main className="page creation-grille-screen">
        ...
        {complete && (
          <>
            <Button
              ...
              variant="secondary"
              color="filledRed"
              disabled={lancementPending}
              onClick={handleLancerPartie}
            >
      ...
      <div className="creation-grille-screen__phrase-container">
        <div className="creation-grille-screen__phrase-title">
          {complete && (
            <p className="creation-grille-screen__complete">
              Ta grille est complète !
            </p>
          )}
          {lancementMessage && (
            <p className="creation-grille-screen__message">
          ...
          {message && (
            <p className="creation-grille-screen__message">{message}</p>
          )}
        ...
        <ul className="phrase-list">
          {phrases.map((phrase) =>
            editingId === phrase.id ? (
              <li
                key={phrase.id}
                className="phrase-list__item phrase-list__item--editing"
              >
                <input
                  className="creation-grille-screen__input"
                  autoFocus
