...
- 2026-07-07 : Ajustement rétrospectif (correct-course) — la borne max de taille de grille passe de 8×8 à 5×5 (décision produit : 25 phrases jugées suffisantes). Migration `20260707210812_reduire_taille_max_grille.sql` (contrainte `taille between 3 and 5`) et `TAILLES` dans `CreationGrilleScreen.tsx` mis à jour en conséquence. Les AC/Tasks ci-dessus reflètent la borne en vigueur au moment de la revue initiale (3-8) et ne sont pas réécrites rétroactivement.
