## Deferred from: code review of spec-fix-debordement-texte-grille (2026-07-30)

- Pas de parité clavier/lecteur d'écran pour la révélation du texte tronqué (`text-reveal-bubble`) : le geste est exclusivement pointer (appui long), aucun `onKeyDown`/`aria-describedby`/`role="tooltip"` — amélioration accessibilité pour un pattern d'interaction entièrement nouveau, pas de précédent dans le projet.
- Pas de `touch-action`/`-webkit-touch-callout: none` sur `.grid-cell` : sur un vrai appareil tactile, un menu contextuel natif (callout copier/partager) ou un geste de scroll peut interrompre l'appui long via `pointercancel` — comportement dégradé mais pas cassé.
- La bulle (`position: fixed`, positionnée une fois à l'ouverture) ne se repositionne pas si la page défile pendant qu'elle est affichée (seul un `resize` la ferme, pas un `scroll`) — fenêtre d'exposition très courte (la bulle se ferme au relâchement du doigt).
- Portée générale du multi-touch sur `GridCell` (au-delà du cas ponctuel de fuite de timer déjà corrigé) : deux pointeurs simultanés sur des cases différentes ne sont pas mutuellement exclusifs — improbable en usage normal (une seule main, un seul doigt à la fois sur un plateau de bingo).
- `friendlyEditErrorMessage` (édition de phrase) ne distingue pas une violation de la contrainte de longueur d'une violation de la contrainte `unique (grille_id, texte)` (ex. : l'utilisateur raccourcit une phrase et le résultat entre en collision avec une autre phrase de la même grille) — retomberait sur le message générique "réessaie dans un instant" alors qu'un nouvel essai identique échouera toujours. Pré-existant (la contrainte unique existait avant ce correctif), simplement mis en lumière par la revue.
- Léger conflit de calque potentiel : `.text-reveal-bubble` (`z-index: 2`) passerait au-dessus de `.vainqueur-overlay` (`z-index: 1`) si les deux étaient affichés simultanément — fenêtre d'occurrence extrêmement étroite (partie qui vient de se terminer pendant qu'un appui long est en cours), pas de priorité.
- `contientPhraseTropLongue` (`src/features/creation-grille/constants.ts`) compare `.length` (unités UTF-16, JS) au lieu de `char_length()` (points de code, Postgres, utilisé par la contrainte SQL) — pour un texte contenant des caractères hors plan de base (emoji notamment), le JS peut compter plus long que Postgres, jamais l'inverse. Conséquence : un blocage de duplication trop prudent sur un cas limite (texte réellement valide au regard de la contrainte SQL), jamais l'échec inverse. Caractéristique systémique du projet (le `maxLength` HTML des champs de saisie a la même limite), pas spécifique à ce correctif — corriger proprement demanderait de compter les points de code partout où une limite de caractères est vérifiée, hors scope d'un correctif de débordement visuel.
